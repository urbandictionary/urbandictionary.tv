describe("voting", function () {
  ...
  it("sends an up-vote", function () {
    ...
    AjaxSpy.find('http://api.urbandictionary.com/v0/vote?defid=' + DOUCHEBAG.defid + '&direction=up').success({status: 'saved'});
  ...
  it("sends an up-vote which is a duplicate, but sets the CSS class anyway", function () {
    ...
    AjaxSpy.find('http://api.urbandictionary.com/v0/vote?defid=' + DOUCHEBAG.defid + '&direction=up').success({status: 'duplicate'});
  ...
  it("sends a down-vote which is a duplicate, but doesn't set the CSS class", function () {
    ...
    AjaxSpy.find('http://api.urbandictionary.com/v0/vote?defid=' + DOUCHEBAG.defid + '&direction=down').success({status: 'duplicate'});
  ...
  it("sends a down-vote and skips to the next video", function () {
    ...
    AjaxSpy.find('http://api.urbandictionary.com/v0/vote?defid=' + DOUCHEBAG.defid + '&direction=down').success({status: 'saved'});
